...
func (r *RingBuffer) PeekUint16() uint16 {
	...
	f, e := r.Peek(2)
	if len(e) > 0 {
		return binary.BigEndian.Uint16(concatBytes(f, e))
	} else {
...
func (r *RingBuffer) PeekUint32() uint32 {
	...
	f, e := r.Peek(4)
	if len(e) > 0 {
		return binary.BigEndian.Uint32(concatBytes(f, e))
	} else {
...
func (r *RingBuffer) PeekUint64() uint64 {
	...
	f, e := r.Peek(8)
	if len(e) > 0 {
		return binary.BigEndian.Uint64(concatBytes(f, e))
	} else {
...
func GetRingBuffer() *RingBuffer {
	return ringBufferPool.Get().(*RingBuffer)
}
...
// makeSpace 确保环形缓冲区有足够的空间容纳新的数据
func (rb *RingBuffer) makeSpace(len int) {
	rb.mu.Lock()
	defer rb.mu.Unlock()
...
// concatBytes 连接两个字节切片
func concatBytes(f, e []byte) []byte {
