...
// parseAddr parses the input address and returns the network type, address, and options.
func parseAddr(addr string) (network, address string, opts addrOpts) {
	network = "tcp"
	address = addr
	opts.reusePort = false

	sepIndex := strings.Index(address, "://")
	if sepIndex != -1 {
		network = address[:sepIndex]
		address = address[sepIndex+3:]
	}
